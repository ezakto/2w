...
(function (window) {
    ...
    var _evt = {
        'modelGet': {
            ...
            cb: function(node, ctrl) {
                if (node === doc.activeElement) return;
                node.value = $2w.prototype.expression.call(ctrl, node.getAttribute(_cfg.formModelAttr));
            }
        },
        'modelSet': {
            nodeType: 1,
        ...
        'click': {
            ...
            event: 'click',
            test: function(node) {
                return node.hasAttribute(_cfg.clickAttr);
            },
            cb: function(evt, node, ctrl) {
                evt.preventDefault();
                $2w.prototype.expression.call(ctrl, node.getAttribute(_cfg.clickAttr));
            }
        }
    };
