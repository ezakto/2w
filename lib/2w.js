...
(function (window) {
     ...
     */
    $2w.prototype._attachController = function (node, ctrl, force, parent) {
        ...
        if (node.nodeType === 1 && node.hasAttributes()) {
            for (i = 0, l = node.attributes.length; i < l; i++) {
                self._attachController(node.attributes[i], ctrl, null, node);
            }
        }
        ...
        // DOM click Attribute -> attach controller
        else if (node.nodeType === 2 && node.nodeName === _cfg.clickAttr) {
            self.addClickListener(parent, node, ctrl);
        }
    ...
    /**
     * Adds click event listeners to dom elements
     * TODO there must be a better approach
     * @param {DOMElement}  node    Input/textarea element
     * @param {Object}      ctrl    Controller object (parent of the @node's model)
     */
    $2w.prototype.addClickListener = function (parent, node, ctrl) {
        var self = this,
            cb = function (evt) {
                evt.preventDefault();
                self.expression.call(ctrl, node.value);
            };
        if (parent.addEventListener) {
            parent.addEventListener('click', cb, false);
        } else {
            parent.attachEvent('onclick', cb);
        }
    };
